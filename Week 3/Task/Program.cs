...
namespace Task
{
    class Program
    {
        class window {
            ...
            public window (FileSystemInfo [] list2) { // initialize values
                list = list2; 
                cursor = 0;
            }
            public void draw() {
                ...
                for (int i = 0; i < list.Length; i++) { // iterating through the list
                    if (i == cursor) 
        ...
        static void Main(string[] args)
        {
            ...
            while (true) {
                ...
                if (k.Key == ConsoleKey.UpArrow) { // if key is upArrow
                    if (cur.cursor > 0) // if it will not go out of the bounderies
                        cur.cursor--; // move cursor up
                }
                if (k.Key == ConsoleKey.DownArrow) { // if key is downArrow
