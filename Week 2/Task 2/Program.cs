...
namespace Task_2
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while ((x = input.ReadLine()) != null) { // reading all lines, where x is the current string that stores a line
                if (isPrime(int.Parse(x))) { // converting x to integer, and checking if it's a prime number
